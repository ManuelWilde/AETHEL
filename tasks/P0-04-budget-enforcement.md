# P0-04: Add BudgetLease Enforcement

## Context

You are working on the AETHEL project — a Rust workspace.
`contracts/src/lib.rs` defines `BudgetLease` (line ~297-313) with fields `tokens_used`, `cost_used_cents`, `max_tokens`, `max_cost_cents` — but these are just data. Nothing prevents an agent from exceeding its budget.
Tasks P0-01 through P0-03 are already completed.

## Git Branch

```bash
git checkout -b P0-04-budget-enforcement
```

## Your Task

1. Create `contracts/src/budget.rs` with enforcement methods for BudgetLease
2. Add `pub mod budget;` to `contracts/src/lib.rs`

## Files to Modify

- `contracts/src/budget.rs` — NEW FILE
- `contracts/src/lib.rs` — add module declaration

## Exact Code

### contracts/src/budget.rs — FULL FILE:
```rust
...
Expected: All tests pass (P0-01 + P0-02 + P0-03 + P0-04 tests), zero warnings.

## Done Criteria

- [ ] `contracts/src/budget.rs` exists
- [ ] `consume()` returns Err on budget exceeded, does NOT modify budget on error
- [ ] `sub_lease()` creates child lease and reserves budget in parent
- [ ] `utilization()` returns 0.0-1.0
- [ ] 17 tests pass
