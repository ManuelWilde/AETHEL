# P0-03: Enforce ClaimState Transitions

## Context

You are working on the AETHEL project — a Rust workspace.
`contracts/src/lib.rs` defines `ClaimState` with 8 variants (Generated, Supported, Accepted, Deferred, Escalated, Revised, Rejected, Retired) but ANY transition is allowed. There is no enforcement.
Tasks P0-01 (AethelError) and P0-02 (IDs) are already completed.

## Git Branch

```bash
git checkout -b P0-03-claim-transitions
```

## Your Task

1. Create `contracts/src/transitions.rs` with transition enforcement for ClaimState
2. Add `pub mod transitions;` to `contracts/src/lib.rs`

## Files to Modify

- `contracts/src/transitions.rs` — NEW FILE
- `contracts/src/lib.rs` — add module declaration

## Exact Code

### contracts/src/transitions.rs — FULL FILE:
```rust
...
### contracts/src/lib.rs — add after existing module declarations:
```rust
pub mod transitions;
```
...
Expected: All 22+ tests pass, zero warnings.

## Done Criteria

- [ ] `contracts/src/transitions.rs` exists
- [ ] `allowed_transitions()` returns correct list for each state
- [ ] `transition()` returns `Err` for invalid transitions
- [ ] All 64 combinations tested (13 valid, 51 invalid)
- [ ] Full lifecycle tests pass (happy path, revision, escalation)
- [ ] `Retired` is terminal, `Generated` is initial

## Git

```bash
git add contracts/src/transitions.rs
git add contracts/src/lib.rs
git commit -m "P0-03: Enforce ClaimState transitions with 22 tests

- Add transitions.rs with allowed_transitions(), can_transition_to(), transition()
- 13 valid transitions, 51 invalid (all 64 combinations tested)
- Lifecycle tests: happy path, revision loop, escalation path
- Retired is terminal, Generated is initial"
```
