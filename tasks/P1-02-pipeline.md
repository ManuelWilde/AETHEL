# P1-02: Pipeline — Sequential Capability Composition

## Prerequisites

P1-01 must be merged to main.
...
P1-01 added: Capability trait, CapValue, CapabilityDescriptor.
Now we build the Pipeline — a sequence of capabilities where the output of one feeds the input of the next.

## Git Branch

```bash
git checkout main && git pull
git checkout -b P1-02-pipeline
```

## Your Task

1. Create `contracts/src/pipeline.rs` with:
   - `PipelineStep` struct (wraps a capability reference with step metadata)
   - `Pipeline` struct (ordered sequence of steps)
   - `Pipeline::validate()` — checks output→input type compatibility
   - `Pipeline::execute()` — runs all steps sequentially, passing CapValue through
2. Add `pub mod pipeline; pub use pipeline::*;` to `contracts/src/lib.rs`
...
### contracts/src/pipeline.rs:
```rust
//! Pipeline — sequential composition of capabilities.
//!
//! A pipeline is an ordered list of capabilities where the output of
//! step N becomes the input of step N+1. Pipelines are validated
//! BEFORE execution (fail-fast) and executed with full tracing.
//!
//! # Design Decisions
//! - Steps hold `Arc<dyn Capability>` for shared ownership
//! - `validate()` is separate from `execute()` — validate once, run many times
//! - Each step records its output type name for debugging
...
pub struct PipelineStep {
    ...
    /// Human-readable label for this step (for tracing/UI).
    pub label: String,
}
...
impl Pipeline {
    ...
    ///
    /// Returns Ok(()) if valid, or Err describing the type mismatch.
    ...
    /// Runs each step sequentially, passing the output of one to the next.
    ///
    /// Returns the final output and a trace of step results.
...
mod tests {
    ...
    async fn test_execute_single_step() {
        ...
        assert_eq!(trace[0].output_type_name, "Text");
    }
    ...
    #[test]
    fn test_pipeline_step_descriptor() {
        let step = PipelineStep::new(Arc::new(UpperCaseCap::new()), "upper");
        assert_eq!(step.descriptor().name, "UpperCase");
    }
}
```
...
Expected: All tests pass (P0 + P1-01 + P1-02), zero warnings.

## Done Criteria

- [ ] `contracts/src/pipeline.rs` exists
- [ ] `Pipeline` struct with id, name, steps
- [ ] `Pipeline::validate()` checks output→input type matching
- [ ] `Pipeline::execute()` runs steps sequentially with tracing
- [ ] `PipelineStep` wraps `Arc<dyn Capability>`
- [ ] `StepResult` traces each step execution
- [ ] 11+ tests pass (empty, single, multi, wrong type, failure propagation, trace)
- [ ] All P0 + P1-01 tests still pass

## Git

```bash
git add -A
git commit -m "P1-02: Pipeline — sequential capability composition

- Pipeline struct with validate() and async execute()
- PipelineStep wraps Arc<dyn Capability> with labels
- StepResult traces execution per step
- Type validation: output→input matching with 'Any' wildcard
- 11+ tests including async execution, failure propagation"
git push -u origin P1-02-pipeline
gh pr create --title "P1-02: Pipeline" --body "$(cat tasks/P1-02-pipeline.md)"
```
